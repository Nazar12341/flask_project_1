...
@app.route("/")
def hello_world():
    return '<h1>Добро пожаловать на мой сайт!</h1><br><a href="/randomfact">Посмотреть случайный факт!</a>' 
...
def password():
    ...
    return f'<p>Ваш случайный пароль: {password}ㅤㅤㅤㅤㅤㅤㅤЧтобы сгенерировать новый, перезагрузите страницу.</p>'
